...
export class SkillManager {
  ...
  buildSkillSourceMeta(sourcePath, sourceMeta = {}) {
    ...
    const inferSourceTypeFromPath = () => {
      const claudeSkillsRoot = path.resolve(path.join(os.homedir(), '.claude', 'skills'))
      const claudeAutoRoot = path.resolve(path.join(claudeSkillsRoot, 'omc-learned'))
      const claudeProjectRoot = path.resolve(path.join(claudeSkillsRoot, 'project-managed'))
      ...
      if (resolved.startsWith(claudeAutoRoot)) {
        return 'claude-auto'
      }
      if (resolved.startsWith(claudeProjectRoot)) {
        return 'claude-project-sync'
      }
      if (resolved.startsWith(claudeSkillsRoot)) {
